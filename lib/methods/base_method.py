...
from lib.core import metric
from lib.core.optim_scheduler import get_optim_scheduler
from lib.utils import gather_tensors_batch, get_dist_info, ProgressBar, GradCAM
...
class Base_method(object):
    ...
    def vis_one_epoch(self, runner, test_loader, vis_path, data_name, layer_id, **kwargs):
        """Visualize the model with test_loader.
        ...
        layer_name = 'hid/enc/{}/block'.format(layer_id)
        grad_cam_layer_ls = [layer_name]

        gradcam = GradCAM(
            self.model,
            target_layers=grad_cam_layer_ls,
            data_mean=test_loader.dataset.mean,
            data_std=test_loader.dataset.std,
            colormap="jet",
        )

        # loop
        for idx, (batch_x, batch_y) in enumerate(test_loader):
            batch_x = batch_x[0].unsqueeze(0)
            batch_y = batch_y[0].unsqueeze(0)
            batch_x, batch_y = batch_x.to(self.device), batch_y.to(self.device)
            # batch_x, batch_y, pred_y => inputs, trues, preds
            localization_maps = gradcam([batch_x], labels=[batch_y])
            npy_file = "{}/feature_npy/{}_layer_{}.npy".format(vis_path, data_name, layer_name.replace("/", "_"))
            npy_data = localization_maps[0].cpu().numpy()
